...
scl::PoissonDiskSampling::PoissonDiskSampling(int width, int height, double min_dist, int point_count) {
	...
	_grid			= std::vector<std::vector<scl::Pos2D_sptr<double>>>(_grid_width, std::vector<scl::Pos2D_sptr<double>>(_grid_height, nullptr));
}
...
std::vector<std::pair<double, double>> scl::PoissonDiskSampling::generate() {
	...
	scl::Pos2D<double> first_point(scl::rand->getDouble(0, _width), scl::rand->getDouble(0, _height));
	...
	_grid[first_point_x][first_point_y] = std::make_shared<scl::Pos2D<double>>(first_point);

	while (!_process.empty()) {
		int new_point_index = scl::rand->getInt(0, static_cast<int>(_process.size()));
		scl::Pos2D<double> new_point = _process[new_point_index];
		_process.erase(_process.begin() + new_point_index);

		for (int i = 0; i < _point_count; i++) {
			scl::Pos2D<double> new_point_around = generatePointAround(new_point);
			...
			if (inRectangle(new_point_around) && !inNeighbourhood(new_point_around)) {
				...
				_grid[new_point_x][new_point_y] = std::make_shared<scl::Pos2D<double>>(new_point_around);
			}
		}
	}
...
scl::Pos2D<double> scl::PoissonDiskSampling::generatePointAround(scl::Pos2D<double> p_point) {
	...
	return scl::Pos2D<double>(new_x, new_y);
}

bool scl::PoissonDiskSampling::inRectangle(scl::Pos2D<double> p_point) {
	return (p_point._x >= 0 && p_point._y >= 0 && p_point._x < _width && p_point._y < _height);
}

bool scl::PoissonDiskSampling::inNeighbourhood(scl::Pos2D<double> p_point) {
	std::vector<scl::Pos2D_sptr<double>> cells = getCellsAround(p_point);
...
std::vector<scl::Pos2D_sptr<double>> scl::PoissonDiskSampling::getCellsAround(scl::Pos2D<double> p_point) {
	std::vector<scl::Pos2D_sptr<double>> cells;
